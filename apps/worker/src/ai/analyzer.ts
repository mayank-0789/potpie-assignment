...
import type { ParsedFile, CodeIssue, IssueType, IssueSeverity } from '@repo/code-review';
 ...
 */
export async function analyzeFile(file: ParsedFile): Promise<CodeIssue[]> {
  try {
    logger.info({ filename: file.filename, language: file.language }, 'Analyzing file with AI');
    ...
    if (!prompt) {
      logger.info({ filename: file.filename }, 'No added lines to analyze');
      return [];
    }
    ...
    const issues = parseAIResponse(response, file.filename);
 ...
 */
function parseAIResponse(response: string, filename: string): CodeIssue[] {
  try {
    ...
    if (!parsed.issues || !Array.isArray(parsed.issues)) {
      logger.warn({ filename }, 'AI response missing issues array');
      return [];
    }
 ...
 */
function validateIssue(issue: any): CodeIssue {
  // Validate type
